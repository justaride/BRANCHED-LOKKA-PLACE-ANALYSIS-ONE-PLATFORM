...
import { loadEiendom } from '@/lib/loaders/brodrene-evensen';
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `${eiendom.adresse} - Brødrene Evensen`,
    description: eiendom.beskrivelse || `Placeanalyse for ${eiendom.adresse}`,
  };
}

export default async function BrodreneEvensenEiendomPage({ params }: PageProps) {
  const { id } = await params;
  const eiendom = await loadEiendom(id);
  ...
  const tilgang = await tilgangsKontekstFraRequest('brodrene-evensen');
        ...
        <Container>
          <FadeIn direction="down">
            <div className="mb-4 md:mb-6">
              <Link
                href="/brodrene-evensen/eiendommer"
                ...
                <div className="md:col-span-2">
                  <div className="rounded-xl border border-gray-200/50 bg-lokka-light/30 p-4 md:rounded-2xl md:p-6">
                    ...
                    <p className="text-xs leading-relaxed text-lokka-secondary md:text-sm">
                      ...
                      Området preges av høy aktivitet og urbant liv i Oslo sentrum.
                    </p>
                  </div>
                </div>
              </div>
