...
import { loadEiendom } from '@/lib/loaders/spabo';
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `${eiendom.adresse} - SPABO Eiendom`,
    description: eiendom.beskrivelse || `Placeanalyse for ${eiendom.adresse}`,
  };
}

export default async function SpaboEiendomPage({ params }: PageProps) {
  const { id } = await params;
  const eiendom = await loadEiendom(id);
  ...
  const tilgang = await tilgangsKontekstFraRequest('spabo');
