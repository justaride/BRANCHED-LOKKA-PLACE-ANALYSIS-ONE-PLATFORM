...
import { loadEiendom } from '@/lib/loaders/eiendomsspar';
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `${eiendom.adresse} - Eiendomsspar`,
    description: eiendom.beskrivelse || `Placeanalyse for ${eiendom.adresse}`,
  };
}

export default async function EiendomssparEiendomPage({ params }: PageProps) {
  const { id } = await params;
  const eiendom = await loadEiendom(id);
  ...
  const tilgang = await tilgangsKontekstFraRequest('eiendomsspar');
