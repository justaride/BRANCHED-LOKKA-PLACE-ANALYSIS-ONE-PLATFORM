...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `${eiendom.adresse} - Roger Vodal`,
    description: eiendom.beskrivelse || `Placeanalyse for ${eiendom.adresse}`,
  };
}

export default async function RogerVodalEiendomPage({ params }: PageProps) {
  const { id } = await params;
  const eiendom = await loadEiendom(id);
                ...
                <div className="md:col-span-2">
                  <div className="rounded-xl border border-gray-200/50 bg-lokka-light/30 p-4 md:rounded-2xl md:p-6">
                    ...
                    <p className="text-xs leading-relaxed text-lokka-secondary md:text-sm">
                      Eiendom på Brenneriveien, Grünerløkka, med utmerket tilgang til kollektivtransport,
                      serveringssteder og byens fasiliteter. Området er kjent for sitt urbane miljø og historiske karakter.
                    </p>
                  </div>
                </div>
              </div>
