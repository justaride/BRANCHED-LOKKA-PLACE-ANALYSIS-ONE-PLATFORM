...
import { loadEiendom, getAllPropertyIds } from '@/lib/loaders/malling-co';
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `${eiendom.adresse} - Malling & Co`,
    description: eiendom.beskrivelse || `Placeanalyse for ${eiendom.adresse}`,
  };
}

export default async function MallingCoEiendomPage({ params }: PageProps) {
  const { id } = await params;
  const eiendom = await loadEiendom(id);
