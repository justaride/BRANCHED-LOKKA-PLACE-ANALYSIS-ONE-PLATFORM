...
Auth: Cloudflare Access (Zero Trust) in front of app | NO in-app login
...
### 5. Authentication (Cloudflare Access — NOT in app code)

All in-app login was removed (commit `10089c8`, June 15 2026). Authentication now
happens **entirely in Cloudflare Access (Zero Trust)** in front of the app:
...
**To grant a person access:** add their email to the Cloudflare Access application
policy in Zero Trust (one.dash.cloudflare.com → Access → Applications → Policies →
Include → Emails). This is a Cloudflare operation, **not a code change or deploy**.

- The `{TENANT}_EMAILS` / `ADMIN_EMAILS` env vars and `src/lib/tenant-emails.ts`
  remain **only** for internal utilities/tests — they do NOT control production access.
- See `docs/architecture/DEPLOYMENT.md` for the current access model.
