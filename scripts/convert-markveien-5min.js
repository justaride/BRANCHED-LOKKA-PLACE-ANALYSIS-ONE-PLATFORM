...
const properties = [
  {
    id: 'markveien-38',
    sourceDir: '/Users/gabrielboen/Downloads/Løkka additions - 09.12.25/Markveien 38',
    outputDir: 'src/data/roger-vodal/markveien-38/5min',
    nokkeltallFile: 'Markveien_38_NOKKELTALL.json',
    aktorerFile: 'Markveien 38 AKTØRLISTE - Sheet1.csv'
  },
  {
    id: 'markveien-42',
    sourceDir: '/Users/gabrielboen/Downloads/Løkka additions - 09.12.25/Markveien 42',
    outputDir: 'src/data/roger-vodal/markveien-42/5min',
    nokkeltallFile: 'Markveien_42_NOKKELTALL.json',
    aktorerFile: 'Markveien 42 Aktørliste - Sheet1.csv'
  },
  {
    id: 'markveien-58',
    sourceDir: '/Users/gabrielboen/Downloads/Løkka additions - 09.12.25/Markveien 58',
    outputDir: 'src/data/roger-vodal/markveien-58/5min',
    nokkeltallFile: 'Markveien_58_NOKKELTALL.json',
    aktorerFile: 'Markveien 58 Aktørliste - Sheet1.csv'
  }
];
...
function convertBesokende(sourceDir) {
  ...
  let aldersFile = path.join(sourceDir, 'Alders- og kjønnsfordeling (besøkende) BESØKENDE.csv');
  if (!fs.existsSync(aldersFile)) {
    aldersFile = path.join(sourceDir, 'Alders- og kjønnsfordeling (besøkende).csv');
  }
  ...
  // Try multiple patterns for husholdninger
  let husholdningerFile = path.join(sourceDir, 'Husholdningstypefordeling (besøkende) BESØKENDE.csv');
  if (!fs.existsSync(husholdningerFile)) {
    husholdningerFile = path.join(sourceDir, 'Husholdningstypefordeling (besøkende).csv');
  }
  const husholdningerContent = readCSV(husholdningerFile);
  ...
  let inntektFile = path.join(sourceDir, 'Inntektsfordeling (besøkende) BESØKENDE.csv');
  if (!fs.existsSync(inntektFile)) {
    inntektFile = path.join(sourceDir, 'Inntektsfordeling (besøkende).csv');
  }
  const inntektContent = readCSV(inntektFile);
  const inntektsfordeling = [];
  ...
  let medianFile = path.join(sourceDir, 'Medianinntekt per husholdningstype (besøkende) BESØKENDE.csv');
  if (!fs.existsSync(medianFile)) {
    medianFile = path.join(sourceDir, 'Medianinntekt per husholdningstype (besøkende).csv');
  }
  const medianContent = readCSV(medianFile);
  const medianInntektPerHusholdningstype = [];
  ...
  // Områder besøkende kommer fra - try multiple patterns
  let områderFile = path.join(sourceDir, 'Omrader_besokende_kommer_fra.BESØKENDEcsv.csv');
  if (!fs.existsSync(områderFile)) {
    områderFile = path.join(sourceDir, 'Omrader_besokende_kommer_fra BESØKENDE.csv');
  }
  const områderContent = readCSV(områderFile);
...
// MAIN
console.log('Converting Markveien 38, 42, 58 to 5-minute analysis JSON...\n');
...
for (const property of properties) {
  ...
  console.log(`  Output: ${property.outputDir}`);
  console.log(`  Summary:`);
